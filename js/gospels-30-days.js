...
(function () {
    ...
    function buildPlan() {
        ...
        for (const b of ALL_BOOKS) {
            if (gospels.includes(b.id)) {
                for (let c = 1; c <= b.chapters; c++) all.push({ bookId: b.id, bookName: b.name, chapter: c });
            }
        }
    ...
    window.PLAN_GOSPELS_30_DAYS = PLAN;

    function getLang() { return window.state?.lang || 'pt'; }
    function getPlanInfo() { return window.PLANS_REGISTRY?.[PLAN_ID] || { name: { pt: 'Evangelhos em 30 Dias' }, tag: { pt: 'EVANGELHOS' }, color: '#f59e0b' }; }
    ...
    function launchConfetti() {
        ...
        const ctx = canvas.getContext('2d'); const COLORS = ['#f59e0b', '#fbbf24', '#facc15', '#f5edd8', '#ffffff'];
    ...
    function openWelcomeScreen() {
        ...
        const description = planInfo.description?.[lang] || planInfo.description?.pt || 'Leia os quatro Evangelhos em 30 dias';
        ...
        content.innerHTML = `
            <div class="daily-wrap fade-in">
                <div class="daily-hero" style="border-color:${planInfo.color}29">
                    <p class="daily-hero-tag">${planInfo.tag?.[lang] || planInfo.tag?.pt || 'EVANGELHOS'}</p>
                ...
                <div class="daily-welcome-cards">
                    ...
                    <div class="daily-welcome-card"><div class="daily-welcome-card-icon"><i class="ph ph-bookmarks"></i></div><div class="daily-welcome-card-val">4</div><div class="daily-welcome-card-label">LIVROS</div></div>
                </div>
    ...
    window.openGospels30DaysWelcome = openWelcomeScreen;
    window.openGospels30DaysDashboard = openDashboard;
    window.openGospels30DaysReading = openReading;

    function buildSidebar() {
        const container = document.getElementById('dailyBooks') || document.getElementById('teensBooks');
        if (!container) return; if (container.querySelector('#gospels30DaysBtn')) return;
        const btn = document.createElement('button'); btn.className = 'book-btn'; btn.id = 'gospels30DaysBtn';
        btn.innerHTML = '<i class="ph ph-sun"></i> Evangelhos em 30 Dias';
