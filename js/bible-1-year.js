 ...
 * Full Bible reading plan - 365 days
 */

(function () {
    'use strict';

    const PLAN_ID = 'bible-1-year';
    const DAYS = 365;
    ...
    window.PLAN_BIBLE_1_YEAR = PLAN;

    // Helper functions
    function getLang() { return window.state?.lang || 'pt'; }
    function getPlanInfo() { return window.PLANS_REGISTRY?.[PLAN_ID] || { name: { pt: 'Bíblia em 1 Ano' }, tag: { pt: 'COMPLETO' }, color: '#8b6f3a' }; }
    ...
    function todayDay() {
        ...
        const diff = Math.floor((new Date() - startDate) / 86400000);
    ...
    function openWelcomeScreen() {
        ...
        const description = planInfo.description?.[lang] || planInfo.description?.pt || 'Leia toda a Bíblia em 365 dias';
        const totalChapters = PLAN.reduce((s, p) => s + p.length, 0);

        let previewRows = '';
        for (let d = 1; d <= 5; d++) {
            const portions = PLAN[d - 1] || [];
            previewRows += `<div class="daily-preview-row"><span class="daily-preview-day">Dia ${d}</span><span class="daily-preview-text">${portions.map(p => `${p.bookName} ${p.chapter}`).join(', ')}</span></div>`;
        }
        ...
        content.innerHTML = `
            <div class="daily-wrap fade-in">
                <div class="daily-hero" style="border-color:${planInfo.color}29">
                    <p class="daily-hero-tag">${planInfo.tag?.[lang] || planInfo.tag?.pt || 'COMPLETO'}</p>
                    <h2 class="daily-hero-title">${planName}</h2>
                    <p class="daily-hero-date">${description}</p>
                    <div class="daily-hero-progress-wrap">
                        <div class="daily-hero-progress-fill" style="width:0%"></div>
                    </div>
                    <div class="daily-hero-progress-row">
                        <span class="daily-hero-daycount">0% concluído</span>
                    </div>
                </div>
                <div class="daily-welcome-cards">
                    ...
                    <div class="daily-welcome-card"><div class="daily-welcome-card-icon"><i class="ph ph-bookmarks"></i></div><div class="daily-welcome-card-val">${ALL_BOOKS.length}</div><div class="daily-welcome-card-label">LIVROS</div></div>
                </div>
                <div class="daily-info-card">
                    <p class="daily-info-title"><i class="ph ph-info"></i> COMO FUNCIONA</p>
                    <p class="daily-info-text">${description} O progresso é salvo automaticamente.</p>
                </div>
                <p class="daily-section-label">PRÉVIA DO PLANO</p>
                <div class="daily-preview-list">${previewRows}
                    <div class="daily-preview-row" style="justify-content:center;opacity:.35;font-size:.72rem;padding:.55rem .9rem">… e mais ${DAYS - 5} dias</div>
                </div>
                <button class="btn-conclude-daily" id="btnStartPlan">
    ...
    function openDashboard(dayNum) {
        ...
        const winEnd = Math.min(DAYS, winStart + 29);
        let dayCards = '';
        ...
        if (allDone) { sc = 'daily-status-done'; sl = 'Completo'; }
        else if (dayNum === today) { sc = 'daily-status-today'; sl = 'Hoje'; }
        else if (dayNum < today) { sc = 'daily-status-late'; sl = 'Atrasado'; }
        ...
        content.innerHTML = `
            <div class="daily-wrap fade-in">
                <div class="daily-hero" style="border-color:${planInfo.color}29">
                    ...
                    <div class="daily-hero-progress-wrap">
                        <div class="daily-hero-progress-fill" style="width:${globalPct}%"></div>
                    </div>
                    <div class="daily-hero-progress-row">
                        <span class="daily-hero-daycount">Dia ${dayNum} de ${DAYS}</span>
                        <span class="daily-hero-pct">${globalPct}%</span>
                    </div>
                </div>
        ...
        document.getElementById('btnResetPlan')?.addEventListener('click', () => {
            if (confirm('Tem certeza que deseja resetar este plano? Todo o progresso será perdido.')) {
                saveCompleted(new Set()); saveStartDate(null); openWelcomeScreen();
            }
        });
        document.getElementById('dailyDaysScroller')?.querySelectorAll('.daily-day-btn').forEach(btn =>
        ...
        document.getElementById('btnStartDay')?.addEventListener('click', () => {
            let target = 0; for (let i = 0; i < portions.length; i++) { if (!completed.has(completedKey(dayNum, portions[i].bookId, portions[i].chapter))) { target = i; break; } } openReading(dayNum, target);
        });
    ...
    async function openReading(dayNum, chapterIdx) {
        ...
        let verses;
        try { const p = portions[chapterIdx]; verses = await fetchChapter(p.bookId, p.chapter); }
        ...
        const verseRows = verses.map(v => '<div class="verse"><span class="verse-num">' + v.verse + '</span><span class="verse-text" style="font-size:' + fontSize + 'rem">' + v.text + '</span></div>').join('');
    ...
    // Public API
    window.openBible1YearWelcome = openWelcomeScreen;
    window.openBible1YearDashboard = openDashboard;
    window.openBible1YearReading = openReading;

    // Sidebar entry
    function buildSidebar() {
        const container = document.getElementById('dailyBooks') || document.getElementById('teensBooks');
        if (!container) return;
        if (container.querySelector('#bible1YearBtn')) return;
        const btn = document.createElement('button');
        btn.className = 'book-btn'; btn.id = 'bible1YearBtn';
        btn.innerHTML = '<i class="ph ph-book-open"></i> Bíblia em 1 Ano';
