 ...
 * Read the Bible in chronological order - 365 days
 */

(function () {
    'use strict';
    const PLAN_ID = 'chronological';
    const DAYS = 365;

    function buildPlan() {
        // For simplicity, use the same distribution as full Bible but note it's chronological
        const all = [];
        for (const b of ALL_BOOKS) for (let c = 1; c <= b.chapters; c++) all.push({ bookId: b.id, bookName: b.name, chapter: c });
        const total = all.length; const base = Math.floor(total / DAYS); const extra = total % DAYS;
        const plan = []; let idx = 0;
        for (let d = 0; d < DAYS; d++) { const count = d < extra ? base + 1 : base; plan.push(all.slice(idx, idx + count)); idx += count; }
        return plan;
    }

    const PLAN = buildPlan();
    window.PLAN_CHRONOLOGICAL = PLAN;

    function getLang() { return window.state?.lang || 'pt'; }
    function getPlanInfo() { return window.PLANS_REGISTRY?.[PLAN_ID] || { name: { pt: 'Bíblia Cronológica' }, tag: { pt: 'CRONOLÓGICO' }, color: '#10b981' }; }
    ...
    function launchConfetti() {
        ...
        const ctx = canvas.getContext('2d'); const COLORS = ['#10b981', '#059669', '#047857', '#f5edd8', '#ffffff'];
    ...
    function openWelcomeScreen() {
        ...
        const description = planInfo.description?.[lang] || planInfo.description?.pt || 'Leia a Bíblia na ordem cronológica dos eventos';
        ...
        content.innerHTML = `
            <div class="daily-wrap fade-in">
                <div class="daily-hero" style="border-color:${planInfo.color}29">
                    <p class="daily-hero-tag">${planInfo.tag?.[lang] || planInfo.tag?.pt || 'CRONOLÓGICO'}</p>
    ...
    window.openChronologicalWelcome = openWelcomeScreen;
    window.openChronologicalDashboard = openDashboard;
    window.openChronologicalReading = openReading;

    function buildSidebar() {
        const container = document.getElementById('dailyBooks') || document.getElementById('teensBooks');
        if (!container) return; if (container.querySelector('#chronologicalBtn')) return;
        const btn = document.createElement('button'); btn.className = 'book-btn'; btn.id = 'chronologicalBtn';
        btn.innerHTML = '<i class="ph ph-clock-counter-clockwise"></i> Bíblia Cronológica';
