...
(function () {
    'use strict';
    const PLAN_ID = 'nt-90-days';
    const DAYS = 90;

    function buildPlan() {
        const ntBooks = BOOKS.nt;
        const all = [];
        for (const b of ntBooks) for (let c = 1; c <= b.chapters; c++) all.push({ bookId: b.id, bookName: b.name, chapter: c });
    ...
    window.PLAN_NT_90_DAYS = PLAN;

    function getLang() { return window.state?.lang || 'pt'; }
    function getPlanInfo() { return window.PLANS_REGISTRY?.[PLAN_ID] || { name: { pt: 'Novo Testamento em 90 Dias' }, tag: { pt: 'NOVO TESTAMENTO' }, color: '#059669' }; }
    ...
    function launchConfetti() {
        ...
        const ctx = canvas.getContext('2d'); const COLORS = ['#059669', '#10b981', '#047857', '#f5edd8', '#ffffff'];
    ...
    function openWelcomeScreen() {
        ...
        const description = planInfo.description?.[lang] || planInfo.description?.pt || 'Leia o Novo Testamento em 90 dias';
        ...
        content.innerHTML = `
            <div class="daily-wrap fade-in">
                <div class="daily-hero" style="border-color:${planInfo.color}29">
                    <p class="daily-hero-tag">${planInfo.tag?.[lang] || planInfo.tag?.pt || 'NOVO TESTAMENTO'}</p>
                ...
                <div class="daily-welcome-cards">
                    ...
                    <div class="daily-welcome-card"><div class="daily-welcome-card-icon"><i class="ph ph-bookmarks"></i></div><div class="daily-welcome-card-val">27</div><div class="daily-welcome-card-label">LIVROS</div></div>
                </div>
    ...
    window.openNT90DaysWelcome = openWelcomeScreen;
    window.openNT90DaysDashboard = openDashboard;
    window.openNT90DaysReading = openReading;

    function buildSidebar() {
        const container = document.getElementById('dailyBooks') || document.getElementById('teensBooks');
        if (!container) return; if (container.querySelector('#nt90DaysBtn')) return;
        const btn = document.createElement('button'); btn.className = 'book-btn'; btn.id = 'nt90DaysBtn';
        btn.innerHTML = '<i class="ph ph-cross"></i> Novo Testamento em 90 Dias';
