...
(function () {
    ...
    function buildPlan() {
        const psalms = ALL_BOOKS.find(b => b.id === 'PSA');
        if (!psalms) return [];
        const all = [];
        for (let c = 1; c <= psalms.chapters; c++) all.push({ bookId: psalms.id, bookName: psalms.name, chapter: c });
    ...
    window.PLAN_PSALMS_30_DAYS = PLAN;

    function getLang() { return window.state?.lang || 'pt'; }
    function getPlanInfo() { return window.PLANS_REGISTRY?.[PLAN_ID] || { name: { pt: 'Salmos em 30 Dias' }, tag: { pt: 'POESIA' }, color: '#3b82f6' }; }
    ...
    function launchConfetti() {
        ...
        const ctx = canvas.getContext('2d'); const COLORS = ['#3b82f6', '#60a5fa', '#93c5fd', '#f5edd8', '#ffffff'];
    ...
    function openWelcomeScreen() {
        ...
        const description = planInfo.description?.[lang] || planInfo.description?.pt || 'Leia todos os Salmos em 30 dias';
        ...
        content.innerHTML = `
            <div class="daily-wrap fade-in">
                <div class="daily-hero" style="border-color:${planInfo.color}29">
                    <p class="daily-hero-tag">${planInfo.tag?.[lang] || planInfo.tag?.pt || 'POESIA'}</p>
    ...
    window.openPsalms30DaysWelcome = openWelcomeScreen;
    window.openPsalms30DaysDashboard = openDashboard;
    window.openPsalms30DaysReading = openReading;

    function buildSidebar() {
        const container = document.getElementById('dailyBooks') || document.getElementById('teensBooks');
        if (!container) return; if (container.querySelector('#psalms30DaysBtn')) return;
        const btn = document.createElement('button'); btn.className = 'book-btn'; btn.id = 'psalms30DaysBtn';
        btn.innerHTML = '<i class="ph ph-music-notes"></i> Salmos em 30 Dias';
