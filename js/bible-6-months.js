/**
 * BIBLE IN 6 MONTHS PLAN
 * Full Bible reading plan - 180 days
 */

(function () {
    'use strict';

    const PLAN_ID = 'bible-6-months';
    const DAYS = 180;
    ...
    window.PLAN_BIBLE_6_MONTHS = PLAN;

    function getLang() { return window.state?.lang || 'pt'; }
    function getPlanInfo() { return window.PLANS_REGISTRY?.[PLAN_ID] || { name: { pt: 'Bíblia em 6 Meses' }, tag: { pt: 'INTENSIVO' }, color: '#b91c1c' }; }
    ...
    function launchConfetti() {
        ...
        const ctx = canvas.getContext('2d'); const COLORS = ['#c9a96e', '#e8d5a8', '#8b6f3a', '#b91c1c', '#f5edd8', '#ffffff'];
    ...
    function openWelcomeScreen() {
        ...
        const description = planInfo.description?.[lang] || planInfo.description?.pt || 'Leia toda a Bíblia em 180 dias';
        ...
        content.innerHTML = `
            <div class="daily-wrap fade-in">
                <div class="daily-hero" style="border-color:${planInfo.color}29">
                    <p class="daily-hero-tag">${planInfo.tag?.[lang] || planInfo.tag?.pt || 'INTENSIVO'}</p>
    ...
    window.openBible6MonthsWelcome = openWelcomeScreen;
    window.openBible6MonthsDashboard = openDashboard;
    window.openBible6MonthsReading = openReading;

    function buildSidebar() {
        const container = document.getElementById('dailyBooks') || document.getElementById('teensBooks');
        if (!container) return; if (container.querySelector('#bible6MonthsBtn')) return;
        const btn = document.createElement('button'); btn.className = 'book-btn'; btn.id = 'bible6MonthsBtn';
        btn.innerHTML = '<i class="ph ph-rocket"></i> Bíblia em 6 Meses';
